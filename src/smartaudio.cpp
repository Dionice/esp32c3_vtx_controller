...
static const uint32_t SA_BITBANG_RESPONSE_WAIT_US = 30000;
static const uint32_t SA_BITBANG_RESPONSE_POLL_US = 5000;
static const uint32_t SA_BITBANG_INTERBYTE_TIMEOUT_US = SA_BIT_US * 6;
...
static bool saRequestMatchesResponse(uint8_t requestCmd, uint8_t responseCmd) {
    switch (responseCmd) {
        case 0x01:
        case 0x09:
        case 0x11:
            return requestCmd == 0x03;
        case 0x02:
            return requestCmd == 0x05;
        case 0x03:
            return requestCmd == 0x07;
        case 0x04:
            return requestCmd == 0x09;
        case 0x05:
            return requestCmd == 0x0B;
        default:
            return false;
    }
}

static bool sa_read_bitbang_byte(uint32_t timeoutUs, uint8_t *byte) {
    if (sa_wire_pin < 0 || byte == NULL) {
        return false;
    }

    const uint32_t startUs = micros();
    while (gpio_get_level((gpio_num_t)sa_wire_pin) != 0) {
    ...
    delayMicroseconds(SA_BIT_US / 2);
    if (gpio_get_level((gpio_num_t)sa_wire_pin) != 0) {
        interrupts();
        return false;
    }
...
static bool sa_read_bitbang_frame(uint8_t *frame, size_t *frameLen, uint32_t waitUs) {
    ...
    while (micros() - startUs < waitUs) {
        const uint32_t remainingUs = waitUs - (micros() - startUs);
    ...
    const size_t totalLength = static_cast<size_t>(5U + frame[3]);
    if (totalLength > 32U) {
        return false;
    }

    for (size_t index = 4; index < totalLength; index++) {
        uint8_t byte = 0;
        if (!sa_read_bitbang_byte(SA_BITBANG_INTERBYTE_TIMEOUT_US, &byte)) {
            return false;
        }
    ...
    *frameLen = totalLength;
    return true;
}

static bool sa_parse_response_frame(const uint8_t *frame, size_t len, uint8_t *responseCmd) {
    if (frame == NULL || responseCmd == NULL || len < 5) {
        return false;
    }
    ...
    if (len != static_cast<size_t>(5U + payloadLength)) {
        return false;
    }

    const uint8_t crc = frame[4 + payloadLength];
    if (crc8_dvb_s2(frame, static_cast<uint8_t>(4 + payloadLength)) != crc) {
        return false;
    }

    const uint8_t *payload = &frame[4];
    if (cmd == 0x01 || cmd == 0x09 || cmd == 0x11) {
        if (payloadLength < 5) {
            return false;
        }
        sa_state.channel = payload[0];
        sa_state.power = payload[1];
        sa_state.opMode = payload[2];
        sa_state.frequency = (static_cast<uint16_t>(payload[3]) << 8) | payload[4];
        sa_state.valid = true;
    } else if (cmd == 0x02) {
        if (payloadLength >= 2) {
            sa_state.channel = payload[0];
        ...
        if (payloadLength >= 2) {
            sa_state.frequency = (static_cast<uint16_t>(payload[0]) << 8) | payload[1];
            sa_state.valid = true;
        }
    } else if (cmd == 0x05) {
        if (payloadLength >= 1) {
            sa_state.opMode = payload[0];
            sa_state.valid = true;
        }
    } else {
        return false;
...
static bool sa_try_read_bitbang_response(uint8_t expectedRequestCmd, uint32_t waitUs) {
    ...
    uint8_t frame[32];
    size_t frameLen = 0;
    if (!sa_read_bitbang_frame(frame, &frameLen, waitUs)) {
        return false;
    }

    uint8_t responseCmd = 0;
    if (!sa_parse_response_frame(frame, frameLen, &responseCmd)) {
        return false;
    }

    return saRequestMatchesResponse(expectedRequestCmd, responseCmd);
}
...
static void sa_handle_incoming(void) {
    if (sa_bitbang_enabled) {
        if (!sa_queue_is_empty() && sa_outstanding != 0) {
            if (sa_try_read_bitbang_response(sa_outstanding, SA_BITBANG_RESPONSE_POLL_US)) {
...
void smartaudioProcess(void) {
    ...
    if (sa_outstanding == 0 && !sa_queue_is_empty()) {
        ...
        if (sa_bitbang_enabled) {
            ...
            if (sa_try_read_bitbang_response(e->cmd, SA_BITBANG_RESPONSE_WAIT_US)) {
                sa_q_head = (sa_q_head + 1) % SMARTAUDIO_QUEUE_DEPTH;
                sa_outstanding = 0;
            }
        } else {
