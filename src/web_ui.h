...
<body>
  ...
  <script>
    ...
    configForm.addEventListener('submit', async (event) => {
      ...
      const data = await response.json();
      const restartRequired = Boolean(data.restartRequired ?? data.restart);
      setStatus(
        data.message || (response.ok ? 'Конфігурацію збережено' : 'Не вдалося зберегти конфігурацію'),
        response.ok ? (restartRequired ? 'Перезапуск' : 'Збережено') : 'Помилка'
      );
      restartPending = response.ok && restartRequired;
      if (restartPending) {
        scheduleReloadAfterRestart();
      }
      await loadState();
