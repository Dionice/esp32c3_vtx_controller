...
constexpr uint32_t kTrampBitUs = 104;
...
static bool trampCommandExpectsResponse(uint8_t command) {
    return command == kTrampCommandGetSettings || command == kTrampCommandGetTemperature;
}
...
static void trampWireIdle(void) {
    if (tramp_wire_pin < 0) {
        return;
    }
    pinMode(tramp_wire_pin, INPUT_PULLUP);
}

static void trampWireBeginTx(void) {
    if (tramp_wire_pin < 0) {
        return;
    }
    gpio_set_level(static_cast<gpio_num_t>(tramp_wire_pin), 1);
    gpio_set_direction(static_cast<gpio_num_t>(tramp_wire_pin), GPIO_MODE_OUTPUT);
    gpio_set_level(static_cast<gpio_num_t>(tramp_wire_pin), 1);
}

static void trampWriteByteBitbang(uint8_t byte) {
    if (tramp_wire_pin < 0) {
        return;
    }
    ...
    gpio_set_level(static_cast<gpio_num_t>(tramp_wire_pin), 0);
    delayMicroseconds(kTrampBitUs);

    for (uint8_t bit = 0; bit < 8; bit++) {
        gpio_set_level(static_cast<gpio_num_t>(tramp_wire_pin), (byte >> bit) & 0x01);
        delayMicroseconds(kTrampBitUs);
    }
...
static void trampSendFrame(uint8_t command, const uint8_t *payload, size_t payloadLength) {
    ...
    if (tramp_wire_pin >= 0) {
        ...
        for (size_t index = 0; index < sizeof(frame); index++) {
            trampWriteByteBitbang(bytes[index]);
        }
        trampWireIdle();
    }

    if (command == kTrampCommandGetLimits) {
...
void trampSetOneWirePin(int pin) {
    tramp_wire_pin = pin;
    if (tramp_wire_pin >= 0) {
        gpio_reset_pin(static_cast<gpio_num_t>(tramp_wire_pin));
        trampWireIdle();
    }
}
...
bool trampInit(void) {
    ...
    delay(kTrampMinRequestPeriodMs);
    return true;
}
