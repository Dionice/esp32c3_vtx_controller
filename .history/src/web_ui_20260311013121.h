...
<body>
  ...
  <script>
    ...
    async function loadTableJson(path = '') {
      const url = path ? `/api/vtx-table/raw?path=${encodeURIComponent(path)}` : '/api/vtx-table/raw';
      const response = await fetch(url);
      if (!response.ok) {
        throw new Error('Failed to load table JSON');
      }
      const text = await response.text();
      const resolvedPath = response.headers.get('X-Vtx-Path') || path || tableSelect.value || jsonTableSelect.value || '';
      jsonEditor.value = text || '';
      savePathInput.value = (resolvedPath || '').replace(/^\//, '');
      if (resolvedPath) {
        jsonTableSelect.value = resolvedPath;
      }
      setStatus(`Loaded ${resolvedPath || 'active table'} into editor`, 'JSON ready');
      return { path: resolvedPath, json: text };
    }
