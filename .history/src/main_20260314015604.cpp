...
static const uint32_t kWirelessHealthCheckIntervalMs = 5000;
static const uint32_t kDeferredConfigSaveDelayMs = 400;
static const uint32_t kDeferredConfigSaveRetryDelayMs = 1000;
...
static unsigned long configSaveAtMs = 0;
...
static void populateVtxCommand(const mavlink_message_t& message,
    ...
    command->powerIndex = static_cast<uint8_t>(constrain(static_cast<int>(lroundf(payload.param5)), 0, 255));
...
static void sanitizeDeviceSelections() {
    config.wifiChannel = static_cast<uint8_t>(constrain(static_cast<int>(config.wifiChannel), 1, 13));
...
static void scheduleConfigSave(uint32_t delayMs = kDeferredConfigSaveDelayMs) {
    configSavePending = true;
    configSaveAtMs = millis() + delayMs;
}

static bool servicePendingConfigSave(bool force = false) {
    if (!configSavePending) {
        return true;
    }

    const unsigned long now = millis();
    if (!force && static_cast<long>(now - configSaveAtMs) < 0) {
        return true;
    }
    ...
    configSaveAtMs = now + kDeferredConfigSaveRetryDelayMs;
    Serial.println("Deferred config save failed; will retry");
    return false;
}
...
static bool forwardMavlinkCommandOverEspNow(const MavlinkVtxCommand& command) {
    if (!espNowActive || !quickEspNow.readyToSendData()) {
        return false;
    }
...
static bool isWirelessApHealthy() {
...
static void resetWirelessStackForAp() {
...
static void initializeWireless() {
    resetWirelessStackForAp();
    espNowActive = false;
    uint8_t apChannel = config.wifiChannel;
    bool apStarted = startSoftApOnChannel(apChannel);
    if (!apStarted) {
        Serial.printf("softAP failed on configured channel %u, retrying once\n", static_cast<unsigned>(apChannel));
        delay(300);
        resetWirelessStackForAp();
        apStarted = startSoftApOnChannel(apChannel);
    }

    if (!apStarted && apChannel != 1) {
        Serial.printf("softAP failed on channel %u, falling back to channel 1\n", static_cast<unsigned>(apChannel));
        delay(300);
        resetWirelessStackForAp();
        apChannel = 1;
        apStarted = startSoftApOnChannel(apChannel);
        if (apStarted) {
            config.wifiChannel = apChannel;
            storageSaveAppConfig(config);
        }
    }
    ...
    if (config.espNowEnabled) {
        ...
        quickEspNow.begin(apChannel, WIFI_IF_AP);
        delay(100);
        if (!isWirelessApHealthy()) {
            Serial.println("ESP-NOW init destabilized AP, restoring AP without ESP-NOW for this boot");
            resetWirelessStackForAp();
            apStarted = startSoftApOnChannel(apChannel);
