  ...
  <script>
    ...
    function formatMavlinkDebug(debug) {
      ...
      const seenText = seen
        ...
        : 'seen msg=n/a';
      const lastText = last
        ? `last cmd=${last.commandId} src=${last.sourceSystem || 0}/${last.sourceComponent || 0} tgt=${last.targetSystem || 0}/${last.targetComponent || 0} node=${last.nodeId} dev=${last.deviceId} band=${last.band} ch=${last.channel} pwr=${last.powerIndex}`
        : 'last cmd=n/a';
      return [
    ...
    function describeDeviceLiveState(device) {
      ...
      return `PWM ${device.currentPwmUs} мкс | Діапазон ${band} | Канал ${channel} | Потужність ${power} | Частота ${frequency}`;
    }
    ...
    function renderDeviceList() {
      ...
      // MAVLink node ID
      rows.push(`<tr><td><strong>MAVLink Node</strong></td>${devicesState.map((d, i) => buildCell(i, `<input class="mavlink-node-input" data-device="${i}" type="number" min="1" max="254" value="${d.mavlinkNodeId}">`)).join('')}</tr>`);
      // MAVLink device ID
      rows.push(`<tr><td><strong>MAVLink Device</strong></td>${devicesState.map((d, i) => buildCell(i, `<input class="mavlink-device-input" data-device="${i}" type="number" min="1" max="254" value="${d.mavlinkDeviceId}">`)).join('')}</tr>`);
      // Table
