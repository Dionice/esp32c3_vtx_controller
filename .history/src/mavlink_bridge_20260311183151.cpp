...
bool MavlinkCommandParser::ingest(uint8_t byte, MavlinkVtxCommand* command) {
    ...
    const bool parsed = parseFrame(command);
    reset();
    return parsed;
}

bool MavlinkCommandParser::parseFrame(MavlinkVtxCommand* command) const {
    if (!command) {
        return false;
    }
    ...
    const uint16_t expectedCrc = mavlinkComputeCrc(buffer_ + 1, headerLength - 1 + payloadLength, kCommandLongCrcExtra);
