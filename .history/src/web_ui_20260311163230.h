    ...
    function renderDeviceList() {
      ...
      devicesState.forEach((device, index) => {
        ...
        const rowHtml = `
          <tr>
            <td>${index + 1}</td>
            <td><input class="name-input" type="text" value="${device.name}"></td>
            <td><input class="pwm-input" type="number" min="0" max="21" value="${device.pwmInputPin}"></td>
            <td><input class="vtx-input" type="number" min="0" max="21" value="${device.vtxControlPin}"></td>
            <td>
              <select class="protocol-input">
            ...
            <td>
              <select class="controlmode-input">
            ...
            <td><select class="table-input">${tableOptions}</select></td>
            <td>
              <select class="enabled-input">
                <option value="1" ${device.enabled ? 'selected' : ''}>Yes</option>
                <option value="0" ${!device.enabled ? 'selected' : ''}>No</option>
              </select>
            </td>
            <td>${serialControlEnabled ? `<select data-role="manual-band">${bandOptions}</select>` : `<select data-role="manual-band" disabled>${bandOptions}</select>`}</td>
            <td>${serialControlEnabled ? `<select data-role="manual-channel">${channelOptions}</select>` : `<select data-role="manual-channel" disabled>${channelOptions}</select>`}</td>
            <td>${serialControlEnabled ? `<select data-role="manual-power">${powerOptions}</select>` : `<select data-role="manual-power" disabled>${powerOptions}</select>`}</td>
            <td>
              <div style="display:flex;gap:6px;align-items:center;">
                <button type="button" class="apply-control-button">Apply</button>
                <button type="button" class="secondary remove-device">Remove</button>
              </div>
            </td>
          </tr>`;
        ...
        const pwmInput = tr.querySelector('.pwm-input');
        const vtxInput = tr.querySelector('.vtx-input');
        const protocolInput = tr.querySelector('.protocol-input');
        const controlModeInput = tr.querySelector('.controlmode-input');
        const tableInput = tr.querySelector('.table-input');
        const enabledInput = tr.querySelector('.enabled-input');
        const manualBandInput = tr.querySelector('[data-role="manual-band"]');
        const manualChannelInput = tr.querySelector('[data-role="manual-channel"]');
        const manualPowerInput = tr.querySelector('[data-role="manual-power"]');
        const applyControlButton = tr.querySelector('.apply-control-button');
        ...
        controlModeInput.addEventListener('change', () => { device.controlMode = controlModeInput.value; renderDeviceList(); });
        tableInput.addEventListener('change', () => { device.vtxTablePath = tableInput.value; });
        enabledInput.addEventListener('change', () => { device.enabled = enabledInput.value === '1'; renderDeviceList(); });
        if (manualBandInput) {
          manualBandInput.addEventListener('change', () => { device.manualBand = Number(manualBandInput.value); renderDeviceList(); });
        }
