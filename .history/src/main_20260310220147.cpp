...
void setup() {
    ...
    // Attempt SmartAudio probe to autodetect VTX
    smartaudio_probe_result_t probe;
    if (smartaudioProbe(&probe, 250)) {
        Serial.println("SmartAudio device detected");
        Serial.print("Channel: "); Serial.println(probe.channel);
        Serial.print("Power: "); Serial.println(probe.power);
        Serial.print("Frequency: "); Serial.println(probe.frequency);
    } else {
        Serial.println("No SmartAudio response (probe timed out)");
    }
}
