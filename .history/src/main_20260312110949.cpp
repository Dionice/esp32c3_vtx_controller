...
static void sendMavlinkCommandAck(const MavlinkVtxCommand& command, uint8_t result) {
    ...
    uint8_t frame[300] = {};
    const size_t frameLength = mavlinkBuildCommandAckFrame(
        command,
        config.localNodeId,
        kMavlinkComponentId,
        result,
...
static void sendMavlinkHeartbeat() {
    ...
    uint8_t frame[300] = {};
    const size_t frameLength = mavlinkBuildHeartbeatFrame(config.localNodeId, kMavlinkComponentId, frame, sizeof(frame));
    if (frameLength == 0) {
        return;
    }
    fcMavSerial.write(frame, frameLength);
