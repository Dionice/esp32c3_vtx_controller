...
static void initializeWireless() {
    ...
    bool apStarted = WiFi.softAP(gApSsid, kApPassword, config.wifiChannel);
    if (!apStarted && config.wifiChannel != 1) {
        Serial.printf("WiFi AP start failed on channel %u, retrying on channel 1\n", config.wifiChannel);
        apStarted = WiFi.softAP(gApSsid, kApPassword, 1);
    }
    Serial.printf("WiFi AP start: %s ssid=%s channel=%u mac=%02X:%02X:%02X:%02X:%02X:%02X ip=%s\n",
                  apStarted ? "ok" : "failed",
                  gApSsid,
