...
#ifndef TEST_MAV_SYS_ID
#define TEST_MAV_SYS_ID 2
#endif

#ifndef TEST_MAV_COMP_ID
#define TEST_MAV_COMP_ID MAV_COMP_ID_TELEMETRY_RADIO
#endif
...
constexpr uint8_t kLocalSystemId = TEST_MAV_SYS_ID;
constexpr uint8_t kLocalComponentId = TEST_MAV_COMP_ID;
...
void sendPingResponse(const mavlink_message_t& sourceMessage, const mavlink_ping_t& ping) {
    mavlink_message_t message = {};
    mavlink_msg_ping_pack_chan(kLocalSystemId,
                               kLocalComponentId,
                               MAVLINK_COMM_0,
                               &message,
                               ping.time_usec,
...
void logPing(const mavlink_message_t& message) {
    mavlink_ping_t ping = {};
    ...
    const bool systemMatches = ping.target_system == 0 || ping.target_system == kLocalSystemId;
    const bool componentMatches = ping.target_component == 0 || ping.target_component == kLocalComponentId;
    if (!systemMatches || !componentMatches) {
        Serial.println("IGNORED PING because target does not match local sysid/compid");
        return;
    }

    sendPingResponse(message, ping);
}
...
void readFlightControllerMavlink() {
    ...
    while (kFlightControllerSerial.available() > 0) {
        const size_t availableBytes = static_cast<size_t>(kFlightControllerSerial.available());
        const size_t bytesToRead = availableBytes < sizeof(buffer) ? availableBytes : sizeof(buffer);
...
void emitSerialHeartbeat() {
    ...
    Serial.printf(" hb=%lu ping=%lu att=%lu gps=%lu sys=%lu rc=%lu text=%lu cmd=%lu req=%lu ack=%lu other=%lu",
                  static_cast<unsigned long>(rxSummary.heartbeat),
                  static_cast<unsigned long>(rxSummary.ping),
...
void setup() {
    ...
    Serial.printf("Active MAVLink identity profile: sys=%u comp=%u\n",
