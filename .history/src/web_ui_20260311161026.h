  ...
  <script>
    ...
    function renderDeviceList() {
      ...
      devicesState.forEach((device, index) => {
        ...
        card.innerHTML = `
          <h3>
            <div style="display:flex;gap:8px;align-items:center;">
              ...
              <span class="meta">${device.name}</span>
            </div>
            <button type="button" class="toggle-btn small">▾</button>
          </h3>
          <div class="device-body">
            <div class="row">
              <div class="field"><input data-role="name" placeholder="Name" value="${device.name}"></div>
              <div class="field"><input data-role="pwm-pin" type="number" min="0" max="21" placeholder="PWM" value="${device.pwmInputPin}"></div>
              <div class="field"><input data-role="vtx-pin" type="number" min="0" max="21" placeholder="VTX" value="${device.vtxControlPin}"></div>
            </div>
            <div class="row">
              <div class="field">
              ...
              <div class="field"><select data-role="vtx-table">${tableOptions}</select></div>
              <div class="field"><select data-role="enabled"><option value="1" ${device.enabled ? 'selected' : ''}>On</option><option value="0" ${!device.enabled ? 'selected' : ''}>Off</option></select></div>
            </div>
            ${serialControlEnabled ? `
            <div class="control-box">
              <div class="status"><strong>Manual</strong></div>
        ...
        const pwmInput = card.querySelector('[data-role="pwm-pin"]');
        const vtxInput = card.querySelector('[data-role="vtx-pin"]');
        const protocolInput = card.querySelector('[data-role="protocol"]');
        const controlModeInput = card.querySelector('[data-role="control-mode"]');
        const tableInput = card.querySelector('[data-role="vtx-table"]');
        ...
        if (nameInput) nameInput.addEventListener('input', () => { device.name = nameInput.value; renderDeviceList(); });
        ...
        if (enabledInput) enabledInput.addEventListener('change', () => { device.enabled = enabledInput.value === '1'; });
        if (manualBandInput) manualBandInput.addEventListener('change', () => { device.manualBand = Number(manualBandInput.value); });
        ...
        if (toggleBtn && body) {
          // collapsed by default; expand toggles class
