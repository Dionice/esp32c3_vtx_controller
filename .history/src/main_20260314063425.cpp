...
static bool isWirelessRequested() {
    return config.webServerEnabled || config.espNowEnabled;
}
...
static void appendStateJson(JsonObject root) {
    ...
    root["webServerEnabled"] = config.webServerEnabled;
    root["webServerRunning"] = webServerRunning;
    ...
    if (mavlinkDebug.hasLastCommand) {
        ...
        lastCommand["powerIndex"] = mavlinkDebug.lastCommand.powerValue;
...
static bool updateConfigFromJson(JsonVariantConst input, bool* restartRequired, String* message) {
    ...
    if (!input["webServerEnabled"].isNull()) {
        updated.webServerEnabled = input["webServerEnabled"].as<bool>();
    }
    ...
    bool needsRestart =
        ...
        updated.webServerEnabled != config.webServerEnabled ||
...
static void initializeWireless() {
    if (!isWirelessRequested()) {
        WiFi.softAPdisconnect(true);
        delay(50);
        WiFi.mode(WIFI_OFF);
        espNowActive = false;
        lastWirelessHealthCheckMs = millis();
        Serial.println("Wireless disabled by config");
        return;
    }
...
void setup() {
    ...
    if (webServerRunning) {
        ...
        Serial.println("Web UI disabled");
    }
}
...
void loop() {
    ...
    if (isWirelessRequested() && millis() - lastWirelessHealthCheckMs >= kWirelessHealthCheckIntervalMs) {
        lastWirelessHealthCheckMs = millis();
        if (!isWirelessApHealthy()) {
