...
bool trampEnqueueGetSettings(void) {
    trampFrame_t f; trampFrameGetSettings(&f);
    return tramp_enqueue((uint8_t*)&f, sizeof(f), 'v');
}

bool trampEnqueueSetFrequency(uint16_t freq) {
    trampFrame_t f; trampFrameSetFrequency(&f, freq);
    return tramp_enqueue((uint8_t*)&f, sizeof(f), 'F');
}

bool trampEnqueueSetPower(uint16_t power) {
    trampFrame_t f; trampFrameSetPower(&f, power);
    return tramp_enqueue((uint8_t*)&f, sizeof(f), 'P');
}
