...
<body>
  ...
  <script>
    ...
    function renderDeviceList() {
      ...
      devicesState.forEach((device, index) => {
        ...
        card.innerHTML = `
          ...
          <div class="status">Pulse ${device.filteredPulse} | Slot ${device.lastMappedIndex}</div>
          <div class="actions">
            <button type="button" class="secondary">Remove device</button>
          </div>
        `;

        const [nameInput, pwmInput, vtxInput, protocolInput, enabledInput] = card.querySelectorAll('input, select');
    ...
    configForm.addEventListener('submit', async (event) => {
      ...
      body.set('selectedVtxPath', configForm.elements.namedItem('selectedVtxPath').value);
