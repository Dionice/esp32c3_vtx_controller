    ...
    function renderDeviceList() {
      ...
      devicesState.forEach((device, index) => {
        ...
        const channelOptions = Array.from({ length: Math.max(1, device.channelCount || 8) }, (_, channelIndex) => {
          ...
          return `<option value="${value}" ${Number(device.manualChannel) === value ? 'selected' : ''}>${value}</option>`;
        }).join('');
        ...
        const serialControlEnabled = device.enabled && device.controlMode === 'serial';
        ...
        card.innerHTML = `
          ...
          <div class="control-box">
            <div class="status"><strong>Manual Control</strong>
Enabled only when control mode is Serial.</div>
            <div class="control-grid">
              <label>Band
                <select data-role="manual-band" ${serialControlEnabled ? '' : 'disabled'}>${bandOptions}</select>
              </label>
              <label>Channel
                <select data-role="manual-channel" ${serialControlEnabled ? '' : 'disabled'}>${channelOptions}</select>
              </label>
              <label>Power
                <select data-role="manual-power" ${serialControlEnabled ? '' : 'disabled'}>${powerOptions}</select>
              </label>
            </div>
            <div class="actions">
              <button type="button" class="apply-control-button" ${serialControlEnabled ? '' : 'disabled'}>Apply serial control</button>
            </div>
          </div>
    ...
    function applyLiveState(state) {
      ...
      liveDevices.forEach((liveDevice, index) => {
        ...
        devicesState[index].controlMode = liveDevice.controlMode || devicesState[index].controlMode;
        devicesState[index].manualBand = Number(liveDevice.manualBand ?? devicesState[index].manualBand);
        devicesState[index].manualChannel = Number(liveDevice.manualChannel ?? devicesState[index].manualChannel);
        devicesState[index].manualPowerIndex = Number(liveDevice.manualPowerIndex ?? devicesState[index].manualPowerIndex);
        devicesState[index].bandOptions = Array.isArray(liveDevice.bandOptions) ? liveDevice.bandOptions : devicesState[index].bandOptions;
        devicesState[index].channelCount = Number(liveDevice.channelCount ?? devicesState[index].channelCount);
        devicesState[index].powerOptions = Array.isArray(liveDevice.powerOptions) ? liveDevice.powerOptions : devicesState[index].powerOptions;
