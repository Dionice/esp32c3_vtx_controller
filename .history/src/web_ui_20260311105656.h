  ...
  <script>
    ...
    function createDevice(device = {}) {
      return {
        ...
        pitMode: typeof device.pitMode === 'boolean' ? device.pitMode : null
      };
    }
    ...
    function describeDeviceLiveState(device) {
      ...
      const pit = device.pitMode == null ? 'n/a' : (device.pitMode ? 'on' : 'off');
      return `PWM ${device.currentPwmUs} us | Band ${band} | Channel ${channel} | Power ${power} | Freq ${frequency} | Pit ${pit}`;
    }
    ...
    function renderDeviceList() {
      ...
      devicesState.forEach((device, index) => {
        ...
        const [nameInput, pwmInput, vtxInput, protocolInput, tableInput, enabledInput] = card.querySelectorAll('input, select');
    ...
    function applyLiveState(state) {
      ...
      liveDevices.forEach((liveDevice, index) => {
        ...
        devicesState[index].pitMode = typeof liveDevice.pitMode === 'boolean' ? liveDevice.pitMode : null;
