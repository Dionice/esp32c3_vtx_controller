...
static const uint8_t SA_CMD_GET_SETTINGS = 0x03;
static const uint8_t SA_CMD_SET_POWER = 0x05;
static const uint8_t SA_CMD_SET_CHANNEL = 0x07;
static const uint8_t SA_CMD_SET_FREQUENCY = 0x09;
static const uint8_t SA_CMD_SET_MODE = 0x0B;

// Betaflight SmartAudio opmode bits
static const uint8_t SA_MODE_SET_IN_RANGE_PITMODE = 1;
static const uint8_t SA_MODE_CLR_PITMODE = 4;
...
static const uint32_t SA_FRAME_GAP_MS = 70;
...
void smartaudioSetOneWirePin(int pin) {
    ...
    if (sa_wire_pin >= 0) {
        ...
        gpio_set_direction((gpio_num_t)sa_wire_pin, GPIO_MODE_INPUT);
    }
}
...
void smartaudioSendPitMode(bool enable) {
    if (enable) {
        // Betaflight uses SET_POWER with 0 dBm | 0x80 for SA v3 pit enable, and SET_MODE for older variants.
        // In this fire-and-forget implementation, send both so the attached VTX can accept whichever it supports.
        smartaudioEnqueueSetPower(0x80);
        smartaudioEnqueueSetMode(SA_MODE_SET_IN_RANGE_PITMODE);
    } else {
        smartaudioEnqueueSetMode(SA_MODE_CLR_PITMODE);
    }
}

bool smartaudioEnqueueGetSettings(void) {
    uint8_t frame[2] = {SA_CMD_GET_SETTINGS, 0};
    return sa_enqueue_raw(frame, 2, SA_CMD_GET_SETTINGS);
}
...
bool smartaudioEnqueueSetChannel(uint8_t band, uint8_t channel) {
    ...
    uint8_t frame[3] = {SA_CMD_SET_CHANNEL, 1, packedChannel};
    return sa_enqueue_raw(frame, 3, SA_CMD_SET_CHANNEL);
}

bool smartaudioEnqueueSetPower(uint8_t power) {
    // SmartAudio expects the raw power value, not a simple local index.
    uint8_t frame[3] = {SA_CMD_SET_POWER, 1, power};
    return sa_enqueue_raw(frame, 3, SA_CMD_SET_POWER);
}
...
bool smartaudioEnqueueSetFrequency(uint16_t frequency, bool pitmodeFrequency) {
    ...
    uint8_t frame[4] = {SA_CMD_SET_FREQUENCY, 2, (uint8_t)((val>>8)&0xFF), (uint8_t)(val & 0xFF)};
    return sa_enqueue_raw(frame, 4, SA_CMD_SET_FREQUENCY);
}

bool smartaudioEnqueueSetMode(uint8_t mode) {
    uint8_t frame[3] = {SA_CMD_SET_MODE, 1, mode};
    return sa_enqueue_raw(frame, 3, SA_CMD_SET_MODE);
}
...
static void sa_handle_incoming(void) {
    ...
    if (smartaudioParseSettingsResponse(sa_rbuf, sa_rpos, &ch,&pw,&op,&freq)) {
        ...
        if (!sa_queue_is_empty()) {
            ...
            if (head->cmd == SA_CMD_GET_SETTINGS || head->cmd == SA_CMD_SET_POWER || head->cmd == SA_CMD_SET_CHANNEL || head->cmd == SA_CMD_SET_FREQUENCY || head->cmd == SA_CMD_SET_MODE) {
