...
void loop() {
    ...
    if (!wifiDisabled && millis() - lastWirelessHealthCheckMs >= kWirelessHealthCheckIntervalMs) {
        lastWirelessHealthCheckMs = millis();
        if (!isWirelessApHealthy()) {
    ...
    if (!wifiDisabled && espNowActive && millis() - lastEspNowBroadcastMs >= 5000 && quickEspNow.readyToSendData()) {
