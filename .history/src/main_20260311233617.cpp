...
static const char* kApSsidPrefix = "ESP32C3-VTX";
static const char* kApPassword = "13579135";
static const char* kEmbeddedDefaultTable = DEFAULT_VTX_TABLE_PATH;
static const unsigned long kSerialAttachWaitMs = 5000;
static const uint8_t kApHidden = 0;
static const uint8_t kApMaxConnections = 4;
static HardwareSerial fcMavSerial(0);
static char gApSsid[32] = {0};
static WiFiEventId_t gWiFiEventHandlerId = 0;
...
static const char* wifiEventToString(WiFiEvent_t event) {
    switch (event) {
...
static void handleWiFiEvent(WiFiEvent_t event) {
    Serial.printf("WiFi event: %s (%d) mode=%d status=%d channel=%d ip=%s ssid=%s\n",
                  wifiEventToString(event),
                  static_cast<int>(event),
                  static_cast<int>(WiFi.getMode()),
                  static_cast<int>(WiFi.status()),
                  static_cast<int>(WiFi.channel()),
                  WiFi.softAPIP().toString().c_str(),
                  gApSsid[0] != '\0' ? gApSsid : "<unset>");
}

static void waitForUsbSerial() {
    const unsigned long startedAt = millis();
    while (!Serial && millis() - startedAt < kSerialAttachWaitMs) {
        delay(10);
    }
    const unsigned long elapsed = millis() - startedAt;
    if (elapsed < kSerialAttachWaitMs) {
        delay(kSerialAttachWaitMs - elapsed);
    }
}

static void logMavlinkCommand(const char* source, const MavlinkVtxCommand& command, bool localApplied, bool forwarded) {
    Serial.printf(
        "MAVLink %s cmd=%u node=%u device=%u band=%u channel=%u power=%u applied=%u forwarded=%u\n",
        source,
...
static void processMavlinkInput() {
    ...
    while (fcMavSerial.available() > 0) {
        ...
        if (!mavlinkParser.ingest(byte, &command)) {
            continue;
        }
...
static void initializeStorageAndConfig() {
    ...
    sanitizeDeviceSelections();
    String message;
    if (!validateDeviceConfigSet(config, &message)) {
        Serial.printf("Stored config invalid, restoring defaults: %s\n", message.c_str());
        appConfigSetDefaults(config);
        sanitizeDeviceSelections();
        storageSaveAppConfig(config);
    }
...
static void initializeWireless() {
    uint8_t mac[6] = {0};
    if (gWiFiEventHandlerId == 0) {
        gWiFiEventHandlerId = WiFi.onEvent(handleWiFiEvent);
    }
    ...
    WiFi.setSleep(false);
    WiFi.macAddress(mac);
    snprintf(gApSsid, sizeof(gApSsid), "%s-%02X%02X%02X", kApSsidPrefix, mac[3], mac[4], mac[5]);
    bool apStarted = WiFi.softAP(gApSsid, kApPassword, config.wifiChannel, kApHidden, kApMaxConnections);
    if (!apStarted && config.wifiChannel != 1) {
        Serial.printf("WiFi AP start failed on channel %u, retrying on channel 1\n", config.wifiChannel);
        apStarted = WiFi.softAP(gApSsid, kApPassword, 1, kApHidden, kApMaxConnections);
    }
    Serial.printf("WiFi AP start: %s ssid=%s hidden=%u max_conn=%u channel=%u mac=%02X:%02X:%02X:%02X:%02X:%02X ip=%s\n",
                  apStarted ? "ok" : "failed",
                  gApSsid,
                  static_cast<unsigned>(kApHidden),
                  static_cast<unsigned>(kApMaxConnections),
                  apStarted && config.wifiChannel != 1 ? config.wifiChannel : (apStarted ? (WiFi.channel() ? WiFi.channel() : 1) : config.wifiChannel),
                  mac[0], mac[1], mac[2], mac[3], mac[4], mac[5],
                  WiFi.softAPIP().toString().c_str());
    const bool shouldStartEspNow = config.espNowEnabled &&
        config.boardRole != BOARD_ROLE_STANDALONE &&
        apStarted;
    if (config.espNowEnabled && config.boardRole == BOARD_ROLE_STANDALONE) {
        Serial.println("ESP-NOW skipped: standalone role does not require it during boot");
    }
    if (shouldStartEspNow) {
        quickEspNow.setWiFiBandwidth(WIFI_IF_AP, WIFI_BW_HT20);
        quickEspNow.onDataRcvd(handleEspNowData);
        const uint8_t espNowChannel = WiFi.channel() > 0 ? WiFi.channel() : config.wifiChannel;
        const bool espNowStarted = quickEspNow.begin(espNowChannel, WIFI_IF_AP) == 0;
        Serial.printf("ESP-NOW start: %s channel=%u\n", espNowStarted ? "ok" : "failed", espNowChannel);
    }
}
