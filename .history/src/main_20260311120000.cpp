...
static bool usesZeroBasedSmartAudioPowerValues(uint8_t deviceIndex) {
    if (deviceIndex >= config.deviceCount || config.devices[deviceIndex].protocol != VTX_PROTOCOL_SMARTAUDIO) {
        return false;
    }
    ...
    if (doc) {
        ...
        for (size_t index = 0; index < levels.size(); index++) {
            if ((levels[index]["value"] | 255) != index) {
                return false;
            }
        }
        return true;
    ...
    for (size_t index = 0; index < (sizeof(kDefaultPowerValues) / sizeof(kDefaultPowerValues[0])); index++) {
        if (kDefaultPowerValues[index] != index) {
            return false;
        }
    }
    return true;
...
static uint8_t getPowerValueForIndex(uint8_t deviceIndex, size_t powerIndex) {
    ...
    if (doc) {
        ...
        if (!levels.isNull() && powerIndex < levels.size()) {
            rawValue = levels[powerIndex]["value"] | 0;
            if (usesZeroBasedSmartAudioPowerValues(deviceIndex)) {
                return static_cast<uint8_t>(powerIndex + 1);
            }
            return rawValue;
        }
    }
    if (powerIndex < (sizeof(kDefaultPowerValues) / sizeof(kDefaultPowerValues[0]))) {
        rawValue = kDefaultPowerValues[powerIndex];
        if (usesZeroBasedSmartAudioPowerValues(deviceIndex)) {
            return static_cast<uint8_t>(powerIndex + 1);
        }
        return rawValue;
...
static void appendSelectionStateJson(uint8_t deviceIndex, JsonObject deviceObject) {
    ...
    if (total == 0 || static_cast<uint32_t>(state.lastMappedIndex) >= total) {
        return;
    }

    uint32_t rem = static_cast<uint32_t>(state.lastMappedIndex);
    const bool pitMode = (rem % 2U) != 0; rem /= 2U;
    const uint32_t powerIndex = rem % powerCount; rem /= powerCount;
    const uint32_t channelIndex = rem % channelCount; rem /= channelCount;
    const uint32_t bandIndex = rem % bandCount;
...
void setup() {
    ...
    for (uint8_t index = 0; index < config.deviceCount; index++) {
        const VtxDeviceConfig& device = config.devices[index];
        Serial.printf("[%u] %s pwm=%u vtx=%u proto=%s table=%s enabled=%u\n", index, device.name, device.pwmInputPin, device.vtxControlPin, appConfigProtocolToString(device.protocol), device.vtxTablePath, device.enabled ? 1 : 0);
    }
