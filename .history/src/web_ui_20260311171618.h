...
static const char WEB_UI_HTML[] PROGMEM = R"с(
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <script>
    ...
    function renderDeviceList() {
      ...
      const headerCells = [`<th>Parameter</th>`, ...devicesState.map((d, i) => `<th>VTX ${i + 1}</th>`)];
      ...
      const rows = [];
      // Name
      rows.push(`<tr><td><strong>Name</strong></td>${devicesState.map((d, i) => buildCell(i, `<input class="name-input" data-device="${i}" type="text" value="${d.name}">`)).join('')}</tr>`);
      // PWM
      ...
      // Protocol
      rows.push(`<tr><td><strong>Protocol</strong></td>${devicesState.map((d, i) => buildCell(i, `<select class="protocol-input" data-device="${i}"><option value="smartaudio" ${d.protocol === 'smartaudio' ? 'selected' : ''}>SmartAudio</option><option value="tramp" ${d.protocol === 'tramp' ? 'selected' : ''}>TRAMP</option></select>`)).join('')}</tr>`);
      // Control Mode
      rows.push(`<tr><td><strong>Control</strong></td>${devicesState.map((d, i) => buildCell(i, `<select class="controlmode-input" data-device="${i}"><option value="pwm" ${d.controlMode === 'pwm' ? 'selected' : ''}>PWM</option><option value="serial" ${d.controlMode === 'serial' ? 'selected' : ''}>Serial</option></select>`)).join('')}</tr>`);
      // Table
      rows.push(`<tr><td><strong>Table</strong></td>${devicesState.map((d, i) => buildCell(i, `<select class="table-input" data-device="${i}">${getTableOptions(d.vtxTablePath).map((p) => `<option value="${p}" ${d.vtxTablePath === p ? 'selected' : ''}>${p}</option>`).join('')}</select>`)).join('')}</tr>`);
      // Enabled
      rows.push(`<tr><td><strong>Enabled</strong></td>${devicesState.map((d, i) => buildCell(i, `<select class="enabled-input" data-device="${i}"><option value="1" ${d.enabled ? 'selected' : ''}>Yes</option><option value="0" ${!d.enabled ? 'selected' : ''}>No</option></select>`)).join('')}</tr>`);
      // Band (manual)
      ...
      // Actions row
      rows.push(`<tr><td><strong>Actions</strong></td>${devicesState.map((d, i) => buildCell(i, `<div style="display:flex;gap:6px;align-items:center;"><button type="button" class="apply-control-button" data-device="${i}">Apply</button><button type="button" class="secondary remove-device" data-device="${i}">Remove</button></div>`)).join('')}</tr>`);
